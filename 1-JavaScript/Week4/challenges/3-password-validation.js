...
const testPassword = (list) => {
list.map(word => { const {times, letter, password} = word;

let testLetter = password.split('').filter((element) => (element === letter)) 

let range   = times.split('');

if (testLetter.length >= range[0] && testLetter.length <= range[2] ){
console.log(`${password} is VALID, ${letter} is present ${testLetter.length} times and should have been present at least ${range[0]} and at most ${range[2]} times`)
} else {
console.log(`${password} is INVALID, ${letter} is present ${testLetter.length} times and should have been present at least ${range[0]} and at most ${range[2]} times`)  
}
});
}

testPassword(passwordList)
